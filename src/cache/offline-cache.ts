...
export default class MyCache extends InMemoryCache {
    ...
    restore(data: NormalizedCacheObject) {
        this.store.dispatch<any>(writeThunk(WRITE_CACHE_ACTION, data));
    ...
    reset() {
        this.store.dispatch<any>(writeThunk(WRITE_CACHE_ACTION, {}));
...
const writeThunk = (type: any, payload: any) => (dispatch) => {
    dispatch({
        type,
        payload,
    });
